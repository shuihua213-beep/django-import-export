...
class ImportExportFormBase(FieldNamePrefixMixin, forms.Form):
    ...
    def _init_formats(self, formats):
        ...
        for i, f in enumerate(formats):
            # Maintain compatibility with `ImportForm` which still passes classes
            title = f.get_title() if not isinstance(f, type) else f().get_title()
            choices.append((str(i), title))
            
        if len(formats) == 1:
            field = self.fields["format"]
            f = formats[0]
            field.value = f.get_title() if not isinstance(f, type) else f().get_title()
            field.initial = 0
